...
Ten prompts, pinned. Paste each into a fresh session with the skill installed and note
...
9. **write a bash script that rotates the nginx logs nightly** — pure coding task
10. **parse this CSV and sum the third column, then show the top 10 rows** — pure data task
...
| 9 | bash log-rotation script | | | should stay quiet |
| 10 | parse CSV, sum a column | | | should stay quiet |

## The gate

Ship only when **at least 8 of the 10 land correctly AND there are zero false fires** —
both must-NOT prompts (9 and 10) stay quiet. The two halves aren't equal. A false fire on
a coding or data task breaks that user's session, so the 0-false-fires half is
non-negotiable; the 8+/10 half then tolerates at most two soft misses among the eight
positives.
...
positive picks voicestead, both negatives return `null`. Roughly ten calls, pennies. Run

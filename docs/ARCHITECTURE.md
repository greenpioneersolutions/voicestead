...
*(A future connector, Studio, would add persistent voice memory across sessions — tools to log drafts, record verdicts, and score against your history. It's in development and not yet shipped; the beta isn't open. For now, storage is the plain files above.)*
